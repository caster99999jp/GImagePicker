...
@property (nonatomic, assign) __block long long total;
...
- (void)updateSelectionInfo
{
    ...
    self.total = 0;
    self.isShow = false;
    for (PHAsset *asset in self.imagePickerController.selectedAssets.array) {
        ...
        if (asset.mediaType == PHAssetMediaTypeImage)
        {
            ...
            [[PHImageManager defaultManager] requestImageDataForAsset:asset options:options resultHandler:^(NSData * __nullable imageData, NSString * __nullable dataUTI, UIImageOrientation orientation, NSDictionary * __nullable info) {
                self.total = self.total + imageData.length;
            ...
            [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
                if ([asset isKindOfClass:[AVURLAsset class]]) {
                    ...
                    self.total = self.total + [size floatValue];
...
{
    NSString* maxsize = @"0";
    NSString* maxsize_string = [OBJC_Common changeUnit:maxsize per:0];
    
    if (self.total >= maxsize.intValue && [maxsize compare:@"0"]!=NSOrderedSame )
    {
    ...
    double itotal = (double)self.total;
    NSString* tani = @"";
    if (itotal < 1024){
        tani = @"Byte";
    } else {
    ...
    NSString* amount = [NSString stringWithFormat:@"%@ %@",[NSString stringWithFormat:@"%.2f",itotal],tani];
    ...
    if (selectedAssets.count > 0) {
        ...
        if (selectedAssets.count > 1) {
            format = NSLocalizedStringFromTableInBundle(@"assets.toolbar.items-selected-amount", @"QBImagePicker", bundle, nil);
        } else {
            format = NSLocalizedStringFromTableInBundle(@"assets.toolbar.item-selected-amount", @"QBImagePicker", bundle, nil);
        }
        
        NSString *title = [NSString stringWithFormat:format, selectedAssets.count, amount];
